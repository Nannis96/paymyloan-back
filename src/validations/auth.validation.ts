...
const totpCode = z.string().trim().min(1, "Code is required").max(64);
...
export const loginTwoFactorSchema = z.object({
  pendingToken: z.string().min(1, "Missing pendingToken"),
  code: totpCode,
});
export type LoginTwoFactorInput = z.infer<typeof loginTwoFactorSchema>;

export const refreshSchema = z.object({
  refreshToken: z.string().min(1, "Missing refreshToken"),
});
