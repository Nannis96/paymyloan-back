...
| `requireContractAccess.ts` | BE-038. Resuelve si la sesión puede acceder a un `Contract` — LENDER (dueño de la `LenderCompany`, comparado contra todas sus empresas) o BORROWER (`ContractBorrower` activo). Consumido por todo `contracts.service.ts`/`contractFees.service.ts` (Fase 6, implementado 2026-09-11). |
