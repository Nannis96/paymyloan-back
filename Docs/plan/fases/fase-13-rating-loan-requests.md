...
>
> **Depende de Fase 6** (`Contract`/`ContractTerms`/`ContractFeeItem`, `BE-051`/`BE-060`/`PB-020`, ya construidos antes de llegar aquí) — corregido 2026-09-10 (`D-S2-19`, autorrevisión post-commit; antes este documento decía "no bloquea ni es bloqueada por Fase 6", que era incorrecto: `PB-017` agrega `Contract.originationSource`/`loanRequestId` vía su propia migración sobre la tabla que Fase 6 ya dejó lista). Lo que sí es cierto: Fase 6 **no** depende de esta fase — es completamente autónoma y se puede dar por cerrada sin que `LoanRequest` exista. Tampoco bloquea Fase 7/11/12 — un `LoanRequest` enganchado produce un `Contract` normal que sigue el mismo ciclo de vida de siempre. Ver [14. Roadmap](../14-roadmap.md) punto 3.
...
- **Prioridad/Complejidad/Dependencias**: P0 / L / PB-011, BE-051+BE-060 (`Contract`/`ContractTerms`, Fase 6, ya construidos), `D-S2-14` (`LoanRequestInvite`)
...
- **Tests**: dos Prestamistas intentando `match` sobre el mismo `LoanRequest` casi simultáneamente — solo uno gana (transacción con verificación de `status` en el mismo `$transaction`, mismo patrón que evita doble-aceptación en `ContractTermsAcceptance`); un Lender con 2 `LenderCompany` que hace match sin `lenderCompanyId` → 400; con un `lenderCompanyId` ajeno → 404.
